...
Write-Verbose "Versioned File Count = $AllVersionFiles.Count" -Verbose

foreach ($file in $AllVersionFiles) 
{ 
	Write-Verbose "Changing AssemblyVersion and AssemblyFileVersion to $assemblyFileVersion on $($file.FullName)" -Verbose

    #version replacements
    (Get-Content $file.FullName) |
    %{$_ -replace 'AssemblyDescription\(""\)', "AssemblyDescription(""assembly built by TFS Build $buildNumber"")" } |
    %{$_ -replace 'AssemblyVersion\("[0-9]+(\.([0-9]+|\*)){1,3}"\)', "AssemblyVersion(""$assemblyFileVersion"")" } |
